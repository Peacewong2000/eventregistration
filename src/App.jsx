...
import { Calendar, Users, Plus, Download, CheckCircle, ClipboardList, Settings, ChevronLeft, Check, LogIn, Search, UserCheck, Lock, LogOut, Key } from 'lucide-react';
...
// --- 元件：頂部導航列 ---
const Header = ({ isAdminAuth, role, setRole, setCurrentView, handleLogout }) => (
  <header className="bg-teal-700 text-white shadow-md">
    <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
      ...
      {isAdminAuth ? (
        <div className="flex bg-teal-800 rounded-lg p-1 items-center">
          <button 
            onClick={() => { setRole('admin'); setCurrentView('list'); }}
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${role === 'admin' ? 'bg-white text-teal-800 shadow' : 'text-teal-100 hover:text-white'}`}
          >
            後台管理 (Admin)
          </button>
          <button 
            onClick={() => { setRole('staff'); setCurrentView('list'); }}
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${role === 'staff' ? 'bg-white text-teal-800 shadow' : 'text-teal-100 hover:text-white'}`}
          >
            現場簽到 (Staff)
          </button>
          <div className="w-px h-5 bg-teal-600 mx-2"></div>
          <button 
            onClick={() => { setRole('participant'); setCurrentView('list'); }}
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${role === 'participant' ? 'bg-white text-teal-800 shadow' : 'text-teal-100 hover:text-white'}`}
          >
            前台預覽
          </button>
          <button 
            onClick={handleLogout}
            className="ml-2 px-2 py-1.5 text-red-300 hover:text-red-100 bg-red-900/30 rounded-md transition flex items-center gap-1"
            title="登出"
          >
            <LogOut size={16} />
          </button>
        ...
        <button 
          ...
          className="flex items-center gap-1.5 bg-teal-800 hover:bg-teal-900 text-teal-100 hover:text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-teal-600"
        >
          <Lock size={16} /> 工作人員登入
        </button>
      )}
...
// --- 工作人員登入面板 ---
const LoginView = ({ setCurrentView, setIsAdminAuth, setRole, showMessage }) => {
  const [password, setPassword] = useState('');
  const ADMIN_PASSWORD = 'ngo'; // 設定您的後台通關密碼
  ...
  const handleLogin = (e) => {
    ...
    if (password === ADMIN_PASSWORD) {
      ...
      setRole('admin');
      setCurrentView('list');
      showMessage('登入成功！歡迎進入後台。');
    } else {
      showMessage('密碼錯誤，請重試。');
      setPassword('');
    }
  };

  return (
    <div className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-100 text-center mt-10">
      <div className="w-16 h-16 bg-teal-100 rounded-full flex items-center justify-center mx-auto mb-4 text-teal-600">
        <Key size={32} />
      </div>
      <h2 className="text-2xl font-bold text-gray-800 mb-2">工作人員登入</h2>
      <p className="text-gray-500 mb-6">請輸入通關密碼以進入後台管理與簽到系統。</p>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        <div>
          <input 
            ...
            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500 text-center text-xl tracking-[0.3em]" 
            placeholder="密碼"
          />
        </div>
        <button type="submit" className="w-full py-3 bg-teal-600 text-white font-bold rounded-lg hover:bg-teal-700 shadow-md transition">
          確認登入
        </button>
        <button type="button" onClick={() => setCurrentView('list')} className="w-full py-2 text-gray-500 hover:text-gray-700 mt-2 text-sm">
          返回活動列表
        </button>
      </form>
    </div>
...
// --- 管理者：新增活動表單 ---
const AdminCreateEvent = ({ setCurrentView, showMessage }) => {
...
// --- 管理者：活動管理與點名 ---
const AdminManageEvent = ({ event, registrations, setCurrentView, showMessage }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      <div className="flex justify-between items-start mb-6">
        ...
        <button onClick={exportToCSV} className="flex items-center gap-2 bg-teal-50 text-teal-700 border border-teal-200 px-4 py-2 rounded-lg hover:bg-teal-100 transition">
...
// --- 參加者：報名表單 ---
const ParticipantRegister = ({ event, setCurrentView, showMessage }) => {
...
// --- 工作人員：現場簽到面板 ---
const StaffCheckIn = ({ event, registrations, setCurrentView, showMessage }) => {
...
export default function App() {
  const [role, setRole] = useState('participant'); // 預設為一般參加者
  const [isAdminAuth, setIsAdminAuth] = useState(false); // 是否已登入後台
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {/* 管理者視角：活動列表 */}
        {isAdminAuth && role === 'admin' && currentView === 'list' && (
          <div>
        ...
        {/* 工作人員視角：活動列表 (選擇簽到活動) */}
        {isAdminAuth && role === 'staff' && currentView === 'list' && (
          <div>
            <div className="mb-6">
        ...
        {/* 渲染子視圖 */}
        {isAdminAuth && role === 'admin' && currentView === 'create' && (
          <AdminCreateEvent setCurrentView={setCurrentView} showMessage={showMessage} />
        )}
        
        {isAdminAuth && role === 'admin' && currentView === 'manage' && (
          <AdminManageEvent event={selectedEvent} registrations={registrations} setCurrentView={setCurrentView} showMessage={showMessage} />
        )}
        
        {role === 'participant' && currentView === 'register' && (
          <ParticipantRegister event={selectedEvent} setCurrentView={setCurrentView} showMessage={showMessage} />
        )}
        
        {isAdminAuth && role === 'staff' && currentView === 'checkin' && (
          <StaffCheckIn event={selectedEvent} registrations={registrations} setCurrentView={setCurrentView} showMessage={showMessage} />
        )}
